...
class DatevExport(models.Model):
    ...
    def _check_partner_data(self, invoice):
        ...
        def check_field(field_value, field_name, partner_name):
            if field_value and re.search(invalid_chars_pattern, field_value):
                ...
                raise ValueError(
                    _(
                        "Partner '%(partner)s' contains invalid characters in "
                        "%(field)s: %(chars)s. These characters cannot be exported "
                        "to DATEV XML format.",
    ...
    def unlink(self):
        ...
        for record in self:
            if hasattr(record, "attachment_id") and record.attachment_id:
